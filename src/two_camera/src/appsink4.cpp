/*
    功能：创建摄像头发布节点 camera2_publisher, 发布主题 camera2_image
    执行：在~/ros2_ws目录下输入：./build/two_camera/appsink2 /dev/video2
*/
...
int main(int argc, char *argv[])
{
    ...
    auto node = rclcpp::Node::make_shared("camera4_10005_publisher");
    auto publisher = node->create_publisher<sensor_msgs::msg::Image>("camera4_10005_image", 10);
    if (!publisher) {
...
static GstFlowReturn new_sample_cb(GstElement *sink, gpointer data)
{
    ...
    if (gst_buffer_map(buffer, &map, GST_MAP_READ)) {
        ...
        msg->header.frame_id = "camera4_10005_frame";
